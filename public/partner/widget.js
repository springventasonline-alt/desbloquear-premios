...
(function(){'use strict';var APP_BASE='https://desbloquear-premios-production.up.railway.app';var CONTAINER_ID='dpp-rewards-bar-root';var POLL_INTERVAL=1500;function getStoreId(){if(window.LS && window.LS.store && window.LS.store.id){return String(window.LS.store.id);}var script=document.currentScript;if(script && script.src){var match=script.src.match(/[?&]store=(\d+)/);if(match)return match[1];}var scripts=document.getElementsByTagName('script');for(var i=scripts.length-1;i>=0;i--){var m=scripts[i].src && scripts[i].src.match(/[?&]store=(\d+)/);if(m)return m[1];}return null;}function formatMoney(cents,currency){var amount=cents/100;var separator=(currency && currency.cents_separator)|| ',';var thousands=(currency && currency.thousands_separator)|| '.';var parts=amount.toFixed(2).split('.');parts[0]=parts[0].replace(/\B(?=(\d{3})+(?!\d))/g,thousands);var formatted=parts.join(separator);if(currency && currency.display_short){return currency.display_short .replace('{{amount}}',formatted).replace('{{amount_no_decimals}}',parts[0]);}return '$'+formatted;}function interpolate(template,vars){return template.replace(/\{\{(\w+)\}\}/g,function(_,key){return vars[key]!=null ? vars[key]:'';});}function escapeHtml(str){return String(str).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g, '&quot;');
  }
  ...
  /** Asegura fondo claro u oscuro con texto legible (independiente del panel del carrito). */
  ...
  function buildStyles(config) {
    ...
    return [
      ...
      '#dpp-rewards-bar-root .dpp-fill{background:linear-gradient(90deg,' + t.primary + ',' + t.accent + ');height:100%;border-radius:999px;transition:width .35s ease}',
      ...
      '#dpp-rewards-bar-root .dpp-level{flex:1;min-width:80px;text-align:center;padding:8px 6px;border-radius:8px;background:' + t.levelBg + ';color:' + t.levelMuted + ';font-size:11px;opacity:1;transition:all .25s ease}',
