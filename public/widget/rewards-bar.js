(function () {
  ...
  function formatMoney(cents, currency) {
    ...
    if (currency && currency.display_short) {
      return currency.display_short.replace('{{amount}}', formatted).replace('{{amount_no_decimals}}', parts[0]);
    }
    return '$' + formatted;
  }
  ...
  function buildStyles(config) {
    return [
      ...
      '.dpp-track { background: rgba(0,0,0,0.08); border-radius: 999px; height: 10px; overflow: hidden; }',
      '.dpp-fill { background: linear-gradient(90deg, ' + config.colors.primary + ', ' + config.colors.accent + '); height: 100%; border-radius: 999px; transition: width 0.35s ease; }',
      '.dpp-message { font-size: 13px; margin-top: 10px; line-height: 1.4; }',
      '.dpp-levels { display: flex; gap: 8px; margin-top: 12px; flex-wrap: wrap; }',
      '.dpp-level { flex: 1; min-width: 80px; text-align: center; padding: 8px 6px; border-radius: 8px; background: rgba(255,255,255,0.6); font-size: 11px; opacity: 0.55; transition: all 0.25s ease; }',
    ...
    var levelsHtml = config.levels.map(function (level) {
      ...
      return (
        '<div class="dpp-level' + (isUnlocked ? ' is-unlocked' : '') + '">' +
          ...
          '<span class="dpp-level-title">' + level.title + '</span>' +
          '<span>' + formatMoney(level.threshold * 100, currency) + '</span>' +
        '</div>'
      );
    }).join('');
